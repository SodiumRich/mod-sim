...
yescounter = np.zeros((runs,Number_Apps))
accept_none = np.zeros((runs,Number_Apps))


#i tracks loops through numbmer of runs
for i in range(runs):
    #Fills applicants with a random sample with no replicates ranging from 1 to Number_Apps
    applicants = random.sample(range(0,Number_Apps),Number_Apps)

    #m tracks loops through each look phase
    for m in range(Number_Apps):
...
axs[0].plot(np.linspace(0, Number_Apps, Number_Apps) , probability)
axs[0].set_title('Probability of Finding Applicant within Top {}'.format(N))
axs[0].set_xlabel('Number in Look Phase')
axs[0].set_ylabel('Probability')


axs[1].plot(np.linspace(0, Number_Apps, Number_Apps) , probability_none)
axs[1].set_title('Probability of Not Accepting Anyone')
